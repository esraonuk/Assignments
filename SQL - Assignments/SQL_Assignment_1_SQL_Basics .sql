...
---- First one sorted the brand_id column in ascending order, then sorted the list_price values ​​of each brand_id value in descending order
---- Second one sorted the list_price column in descending order, then sorted the brand_id values ​​of each list_price value in ascending order
...
SELECT *
FROM sale.customer
WHERE last_name LIKE 'B%' and first_name LIKE '%s'
